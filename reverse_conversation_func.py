...
def reverse_conversation_to_features(
    ...
    if is_single_turn_conversation(messages):
        ...
        # After reordering, we need to find the user message span (now in position 2)
        # and unmask it for loss calculation
        
        # Find where the user message is (should be the last message after reordering)
        user_message_idx = -1
        ...
        if user_message_idx >= 0:
            # Get the span for the user message
            # Start position: end of previous message (or 0 if it's the first)
            start_pos = prefix_lengths[user_message_idx - 1] if user_message_idx > 0 else 0
            # End position: end of current message
            end_pos = prefix_lengths[user_message_idx]
            
            # Unmask the user message tokens
            for i in range(start_pos, min(end_pos, len(labels))):
                labels[i] = full_ids[i]
    else:
        # ------------------------------------------------------------------
        ...
        # For multi-turn, we need to unmask only user messages
        # Iterate through all messages and identify user turns
        
        for i, msg in enumerate(messages):
            if msg["role"] == "user":
                # Get the span for this user message
                # Start position: end of previous message (or 0 if first)
                start_pos = prefix_lengths[i - 1] if i > 0 else 0
                # End position: end of current message (handle truncation)
                end_pos = min(prefix_lengths[i], len(full_ids))
